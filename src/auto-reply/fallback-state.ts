...
export function formatFallbackAttemptReason(attempt: RuntimeFallbackAttempt): string {
  ...
  if (typeof attempt.status === "number") {
    if (attempt.status === 529) {return "model is busy";}
    if (attempt.status === 429) {return "model is busy";}
    if (attempt.status === 503) {return "temporarily unavailable";}
    if (attempt.status === 502) {return "temporarily unavailable";}
